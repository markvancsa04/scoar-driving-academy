...
export function VehicleCard({ vehicle }: { vehicle: Vehicle }) {
  const imageUrl = resolveMediaUrl(vehicle.image);
  const hasName = Boolean(vehicle.name);
  const hasSpecRow = Boolean(vehicle.transmission) || Boolean(vehicle.fuel);
  const hasDesc = Boolean(vehicle.description);
    ...
    <article className="group h-full overflow-hidden rounded-3xl border border-primary-foreground/10 bg-primary-foreground/[0.04] transition-all duration-300 hover:-translate-y-1 hover:border-accent/40">
      <div className="overflow-hidden">
        {imageUrl ? (
          ...
          <div
            role="img"
            aria-label={vehicle.image?.alt || undefined}
